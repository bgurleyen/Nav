...
namespace Navigation
{
    public class DataHandler
    {
        ...
        private static void ComputeRegulationsInterval(List<AltitudeRegulationNode> regulations,
            RouteScriptableObject set)
        {
                ...
                if (anchored)
                {
                        ...
                        validationCursor.LinkedPoint.AltitudeRegulation != RoutePoint.AltitudeFlags.Exact)
                    {
                        ...
                        do
                        {
                            ...
                            var validationValueModified =
                                computedToCheck != validationCursor.LinkedPoint.GetAcceptedAltitude;
                            ...
                            if (validationAnchored && validationUpdated)
                            {
                                ...
                                ComputeStraightInterval(validationCursor,
                                    validationAnchoredTo); // may not be needed - it's direct
                            }
            ...
            void ComputeStraightInterval(AltitudeRegulationNode from, AltitudeRegulationNode to)
            {
                ...
                for (var i = from.IndexInList + 1; i <= to.IndexInList; i++)
                {
                    ...
                    regulation.LinkedPoint.Altitude.SetComputedValue(set.Points[i - 1].GetAcceptedAltitude + dif,
                        false);
                }
            }
            ...
            while (anchoredFrom.Next != null)
            {
                ...
                var totalDiff = anchoredFrom.Next.LinkedPoint.GetAcceptedAltitude -
                                anchoredFrom.LinkedPoint.GetAcceptedAltitude;
                var lossForMile = totalDiff / totalDistance;
