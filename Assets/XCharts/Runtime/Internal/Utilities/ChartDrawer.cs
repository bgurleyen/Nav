            ...
            Vector3 startPos, float symbolSize2 = 0f)
        {
            switch (type)
            {
                ...
                case SymbolType.DownArrow:
                    if (gap > 0)
                    {
                        SVG.DrawDownArrow(vh, pos - (Vector3.right * (symbolSize *270)), symbolSize + gap, backgroundColor);
                    }
                    SVG.DrawDownArrow(vh, pos - (Vector3.right * (symbolSize * 270)), symbolSize + gap, color);
                    break;
                case SymbolType.UpArrow:
                    if (gap > 0)
                    {
                        SVG.DrawUpArrow(vh, pos - (Vector3.right * (symbolSize * 270)), symbolSize + gap, backgroundColor);
                    }
                    SVG.DrawUpArrow(vh, pos - (Vector3.right * (symbolSize * 270)), symbolSize + gap, color);
                    break;
            }
        }
