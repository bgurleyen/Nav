...
namespace XUGL
{
    public class SVGPath
    {
        ...
        public void Draw(VertexHelper vh, Vector2 offset, float scale, Color color)
        {
            var sp = Vector2.zero;
            var np = Vector2.zero;
            var posList = new List<Vector3>();
            ...
            foreach (var seg in segs)
            {
                switch (seg.type)
                {
                    case SVGPathSegType.M:
                        ...
                        if (posList.Count > 0)
                        {
                            DrawPosList(vh, posList, offset, scale, color, isClosed);
                        }
                        posList.Add(np);
                        isClosed = false;
                        break;
                    ...
                    case SVGPathSegType.A:
                    default:
                        Debug.LogError("unknown seg:" + seg.type);
                        break;
                }
            }
            if (posList.Count > 0)
        ...
        /*private void DrawPosList(VertexHelper vh, List<Vector3> posList, Vector2 offset, float scale)
        {
            if (posList.Count == 0) return;

            // Apply mirrorY, scale, and offset transformations
            for (int i = 0; i < posList.Count; i++)
            {
                ...
                // Apply scale
                pos = new Vector3(pos.x * scale, pos.y * scale, pos.z);
                // Apply offset
                pos = new Vector3(pos.x + offset.x, pos.y + offset.y, pos.z);
                posList[i] = pos;
            }
        ...
        private void DrawPosList(VertexHelper vh, List<Vector3> posList, Vector2 offset, float scale, Color color, bool isClosed)
        {
            if (posList.Count < 2) return;

            var transformedPosList = new List<Vector3>();
            for (int i = 0; i < posList.Count; i++)
            {
                var pos = posList[i];
                if (mirrorY)
                ...
                pos = new Vector3(pos.x * scale, pos.y * scale, pos.z);
                pos = new Vector3(pos.x + offset.x, pos.y + offset.y, pos.z);
                transformedPosList.Add(pos);
            }

            if (isClosed && transformedPosList.Count >= 3)
            {
                int vertexStartIndex = vh.currentVertCount;
                for (int i = 0; i < transformedPosList.Count; i++)
                {
                    vh.AddVert(transformedPosList[i], color, Vector2.zero);
                }
                for (int i = 1; i < transformedPosList.Count - 1; i++)
                {
                    vh.AddTriangle(vertexStartIndex, vertexStartIndex + i, vertexStartIndex + i + 1);
                }
            }
            UGL.DrawLine(vh, transformedPosList, 1f, color, false);
            posList.Clear();
        }
    }
}