...
{
    private RouteScriptableObject Route;
    ...
    private Vector2 pointPos(int pt)
    {
    ...
    public IEnumerator MoveMyAC()
    {
...

        {
                ...
                || ((RawSpeed > 0) && (RawSpeed < ATCSpeed) && Cmode == 1)) //Speed clr changed
            {
                ...
                InvokeRepeating("SpeedCheck", Mathf.Abs((float)Calculator.CSpeed - ATCSpeed) * 2.5f,
    ...
    { //Moves  other ACs based on the Route and Alt(Altitude) arrays.
        ...
        if (otherACLevel[Level].otherACnr.Length <= ACnr)
        {
            yield break;
        }
